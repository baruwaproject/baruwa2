    ...
    if data.success
        ...
        $.ajax url + '.json',
            ...
            success: (data, textStatus, XHR) ->
                d = `new Date()`
                rows = []
                tmpl = '<tr><td>{{counter}}.</td><td><span class="label label-{{counter}}">' +
                        '&nbsp;&nbsp;</span>&nbsp;{{address}}</td><td class="hidden-phone">' +
                        '{{count}}</td><td class="hidden-phone">{{size}}</td></tr>'
                $.each data.items, (i, f)->
                    item = {}
                    item['counter'] = i + 1
                    item['address'] = f[0]
                    item['count'] = f[1]
                    item['size'] = filesizeformat f[2]
                ...
                if rows.length
                    ...
                    replacement = '<tr><td colspan="4">'+text+'</td></tr>'
                $('#pietbody').empty().append(replacement)
                ndata = convert_data data.pie_data
                generate_pie ndata
                1
